    ...
    **kwargs
    ...
    if particles[0].dim == 2:
                ...
                for k in range(-1, 2):
                    if (
                        ...
                        circ = mpatches.Circle(
                            particles[i].position_center_history[iteration]
                        ...
                        if verlet_ngh:
                            circ = mpatches.Circle(
                                ...
                                * np.array(
                                    [1 * j, 1 * k]
                                    + particles[i].displacement_last_verlet
                                ),
                                radius=Particle.verlet_factor * particles[i].radius,
                                alpha=0.1,
                                color=colors[particles[i].phase],
                            )
                            ax.add_artist(circ)
                        if center_part:
                            plt.annotate(
                                xy=particles[i].position_center_history[iteration],
                                s=str(i),
                            )
                            plt.scatter(
                                particles[i].position_center_history[iteration][0],
                        ...
                        if verlet_ngh:
                            ellip = mpatches.Ellipse(
                                ...
                                * np.array(
                                ...
                                particles[i].major_axis * Particle.verlet_factor,
                                particles[i].minor_axis * Particle.verlet_factor,
                                angle=180 / np.pi * particles[i].angle,
                                alpha=0.2,
                                color=colors[particles[i].phase],
                            )
        ...
        if grid == "cell_list":
            ...
            plt.xticks(
                np.linspace(
                    0,
                    1,
                    discret_spec_array["rgmsh"]["n_voxels_dims"][0] + 1,
                    endpoint=True,
                )
            )
            plt.yticks(
                np.linspace(
                    0,
                    1,
                    discret_spec_array["rgmsh"]["n_voxels_dims"][1] + 1,
                    endpoint=True,
                )
            )
            plt.grid(b=True, which="both")
        ...
        boxTag = factory.addBox(
            0, 0, 0, Particle.box[0], Particle.box[1], Particle.box[2]
        )
                ...
                for p in range(-1, 2):
                    ...
                    for l in range(-1, 2):
                        ...
                        if "Sphere" == class_name_i_particle:
                            ...
                            xc = (
                                i_particle.position_center_history[iteration][0]
                                + Particle.box[0] * j
                            )
                            yc = (
                                i_particle.position_center_history[iteration][1]
                                + Particle.box[1] * p
                            )
                            zc = (
                                i_particle.position_center_history[iteration][2]
                                + Particle.box[2] * l
                            )
                            r = i_particle.radius
                            # Saving the properties of the particles
        ...
        for i_phase in range(len(Particle.list_phases)):
            materialTag = model.addPhysicalGroup(
                2, [particle[1] for particle in materials[i_phase]]
            )
            model.setPhysicalName(
                2, materialTag, "Phase " + Particle.list_phases[i_phase]
            )
...
def plotKineticEnergyHistory(kinetic_energy_history, save=True, show=False, **kwargs):
    if "axes" in kwargs:
        ...
        plt.figure()
    plt.plot(
        range(len(Particle.kinetic_energy_history)), Particle.kinetic_energy_history
    )
...
def plotPaths(particles, dim, dp_dir):
    ...
    if dim == 3:
        ...
        for i_particle in particles:
            ...
            c = np.array(
                [
                    np.concatenate((color, np.array([1])))
                    for i in range(len(path_i[:, 0]))
                ]
            )
        ...
        # Slider
        samp = Slider(
            axamp, "Frame", 0, len(particles[0].position_center_history) - 1, valinit=0
        )
        ...
        def update_plot_3D(num):  # , *args):
            ...
            samp.set_val(val)
            is_manual = (
                False  # the above line called update_slider, so we need to reset this
            )
        ...
        ani = animation.FuncAnimation(
            fig, update_plot_3D, frames=len(particles) - 1, blit=False
        )
        ...
        if only_center:
            ...
            for i in range(len(particles)):
                for j in range(-1, 2):
                    for k in range(-1, 2):
                        if (
                            particles[i].__class__.__name__ == "Disk"
                            or "CylindricalFiber" == particles[i].__class__.__name__
                        ):
                            circ = mpatches.Circle(
                                np.array(particles[i].position_center_history)[0, :]
                                + Particle.box * np.array([1 * j, 1 * k]),
                                radius=particles[i].radius,
                                alpha=0.5,
                                color=colors[particles[i].phase],
        ...
        def update(frame):
            ...
            if only_center:
                ...
                for i in range(len(particles)):
                    for j in range(-1, 2):
                        for k in range(-1, 2):
                            particle_patches[9 * i + 3 * (j + 1) + (k + 1)].set_center(
                                np.array(particles[i].position_center_history)[
                                    int(frame), :
                                ]
                                + Particle.box * np.array([1 * j, 1 * k])
                            )
        ...
        ani = animation.FuncAnimation(
            fig, update_plot, 500
        )  # len(particles[0].position_center_history)) #, fargs=(is_manual, is_manual), interval=interval)
...
def plotVoronoi3Dpbc(
    ...
    outDimTag, outDimTagMap = factory.intersect(
        [(3, boxTag)],
        [(2, particleTag) for particleTag in particleTags],
        removeObject=False,
        removeTool=True,
    )
